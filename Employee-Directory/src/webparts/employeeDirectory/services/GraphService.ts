...
export class GraphService {
  ...
  /**
   * Get all employees with progressive loading (without photos for performance)
   */
  public async getAllEmployees(
    groupId: string,
    onProgress?: (employees: IEmployee[]) => void
  ): Promise<IEmployee[]> {
    try {
      ...
      while (nextLink) {
        ...
        const newEmployees = users.map(this.mapToEmployee);

        allEmployees.push(...newEmployees);

        // Send progress update
        if (onProgress) {
          onProgress([...allEmployees]);
        }
      ...
      // Fetch manager IDs
      await this.fetchManagerIds(allEmployees);

      // Calculate direct reports
      this.calculateDirectReports(allEmployees);
   ...
   * 单独获取单个员工的照片（用于模态框）
   */
  public async getEmployeePhoto(employeeId: string): Promise<string | undefined> {
    try {
      ...
      console.log(`Photo not available for employee ${employeeId}`);
    }
    return undefined;
  }
...
//   /**
//    * Get all employees with progressive loading and photos
//    */
//   public async getAllEmployees(
//     groupId: string,
//     onProgress?: (employees: IEmployee[]) => void
...
//       // Fetch photos for all employees progressively
//       await this.fetchEmployeePhotos(allEmployees, onProgress);
...
//   /**
//    * Fetch photos for employees progressively
//    */
...
//   //     // Process photos in parallel for this batch
//   //     await Promise.all(
//   //       batch.map(async (employee) => {
//   //         try {
//   //           // Use the correct photo endpoint without responseType
//   //           const photoResponse = await this.graphClient
...
//   //           }
//   //         } catch (error) {
//   //           // Photo not available, use default (no photo)
...
//   /**
//  * Fetch photos for employees progressively - 只在模态框需要时获取
//  */
// private async fetchEmployeePhotos(
//   employees: IEmployee[],
//   onProgress?: (employees: IEmployee[]) => void
// ): Promise<void> {
//   // 不在主列表中加载照片，只在模态框需要时加载
//   // 这样可以提高初始加载性能
//   console.log('Skipping photo loading for main list to improve performance');
