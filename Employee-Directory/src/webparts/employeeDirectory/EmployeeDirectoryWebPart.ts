...
export default class EmployeeDirectoryWebPart extends BaseClientSideWebPart<IEmployeeDirectoryWebPartProps> {
  ...
  private loadingEmployeeId: string | null = null;
    ...
    return `
      <div class="${styles.employeeDirectory}">
        <div class="${styles.emptyState}">
          <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#0078d4" stroke-width="2">
            <path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path>
            <circle cx="9" cy="7" r="4"></circle>
            <path d="M23 21v-2a4 4 0 0 0-3-3.87"></path>
            <path d="M16 3.13a4 4 0 0 1 0 7.75"></path>
          </svg>
          <h2>No Azure AD Group Selected</h2>
          <p>Please configure the web part and select an Azure AD group to display employees.</p>
          <button id="configureButton" class="${styles.primaryButton}">
            Configure Web Part
          </button>
        </div>
      </div>
        ...
        <div class="${styles.filterContainer}">
          <div class="${styles.filterRow}">
            <div class="${styles.searchBar}">
              <svg class="${styles.searchIcon}" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          ` : ''}
            ...
            <div class="${styles.resultsCount}">
              Showing <strong>${this.displayedEmployees.length}</strong> of ${this.filteredEmployees.length} employees
            </div>
          </div>
  ...
  private getEmployeeCardHTML(employee: IEmployee): string {
    ...
    const isLoading = this.loadingEmployeeId === employee.id;

    return `
      <div class="${styles.employeeCard}" data-employee-id="${employee.id}">
        ${isLoading ? `
          <div class="${styles.cardLoadingOverlay}">
            <div class="${styles.spinner}"></div>
            <div class="${styles.loadingText}">Loading profile...</div>
          </div>
        ` : ''}
        ...
        <div class="${styles.cardBody}">
          <div class="${styles.employeeDetail}">
            ...
            <span>${employee.officeLocation || 'N/A'}</span>
          </div>
          <div class="${styles.employeeDetail}">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          <div class="${styles.modalBody}">
            ...
            <div class="${styles.modalProfileHeader}">
              <div class="${styles.modalAvatar}">
                ${employee.photo
                  ? `<img src="${employee.photo}" alt="${employee.displayName}" class="${styles.avatarImage}" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';" />`
                  : ''
                }
                <div class="${styles.avatarFallback}" ${employee.photo ? 'style="display: none;"' : ''}>
                  ${initials}
                </div>
              </div>
              <div class="${styles.modalProfileInfo}">
                <h3>${employee.displayName}</h3>
                <p class="${styles.modalJobTitle}">${employee.jobTitle || 'N/A'}</p>
                <div class="${styles.modalBadges}">
                  <span class="${styles.badge}">${employee.department || 'N/A'}</span>
                  <span class="${styles.badge} ${styles.badgeOutline}">${employee.officeLocation || 'N/A'}</span>
                </div>
              </div>
            </div>
              ...
              <div class="${styles.modalContactGrid}">
                ...
                <div class="${styles.modalContactItem}">
                  ...
                  <div>
                    ...
                    <p>${employee.officeLocation || 'N/A'}</p>
                  </div>
                </div>
              </div>
            </div>
                  ...
                  ${manager ? `
                    <div class="${styles.modalManagerInfo}">
                      <div class="${styles.modalSmallAvatar}">
                        ${manager.photo
                          ? `<img src="${manager.photo}" alt="${manager.displayName}" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';" />`
                          : ''
                        }
                        <div class="${styles.avatarFallback}" ${manager.photo ? 'style="display: none;"' : ''}>
                          ${manager.givenName?.charAt(0) || ''}${manager.surname?.charAt(0) || ''}
                        </div>
                      </div>
                      <div>
                        <p class="${styles.modalManagerName}">${manager.displayName}</p>
                        <p class="${styles.modalManagerTitle}">${manager.jobTitle || 'N/A'}</p>
                      </div>
                    </div>
                ...
                <div class="${styles.modalTeamList}">
                  ${directReports.map(report => `
                    <div class="${styles.modalTeamMember}">
                      <div class="${styles.modalSmallAvatar}">
                        ${report.photo
                          ? `<img src="${report.photo}" alt="${report.displayName}" onerror="this.style.display='none'; this.nextElementSibling.style.display='flex';" />`
                          : ''
                        }
                        <div class="${styles.avatarFallback}" ${report.photo ? 'style="display: none;"' : ''}>
                          ${report.givenName?.charAt(0) || ''}${report.surname?.charAt(0) || ''}
                        </div>
                      </div>
                      <div>
                        <p class="${styles.modalTeamMemberName}">${report.displayName}</p>
                        <p class="${styles.modalTeamMemberTitle}">${report.jobTitle || 'N/A'}</p>
                      </div>
                    </div>
                  `).join('')}
                </div>
  ...
  private attachEventListeners(): void {
    ...
    const configureButton = this.domElement.querySelector('#configureButton');
    if (configureButton) {
  ...
  private async openModal(employeeId: string): Promise<void> {
    ...
    if (employee) {
      ...
      if (!employee.photo) {
        try {
          employee.photo = await this.graphService.getEmployeePhoto(employeeId);
        } catch (error) {
          console.error('Error loading employee photo:', error);
        }
      }

      // Clear loading state and open modal
  ...
  private async loadEmployees(): Promise<void> {
    try {
      // Clear all existing employees immediately when loading new group
      this.employees = [];
      ...
      // Load employees without photos for better performance
      await this.graphService.getAllEmployees(
        this.properties.selectedGroupId,
        (employees) => {
          // Update employees list as they come in
  ...
  protected async onPropertyPaneFieldChanged(propertyPath: string, oldValue: any, newValue: any): Promise<void> {
    if (propertyPath === 'selectedGroupId' && oldValue !== newValue) {
      if (newValue) {
        ...
        // If group is cleared, reset everything
        this.employees = [];
        this.filteredEmployees = [];
        this.displayedEmployees = [];
        this.currentPage = 1;
...
//           <h2>No Azure AD Group Selected</h2>
//           <p>Please configure the web part and select an Azure AD group to display employees.</p>
//           <button class="${styles.primaryButton}" onclick="window.dispatchEvent(new CustomEvent('openPropertyPane'))">
//             Configure Web Part
//           </button>
//         </div>
//       </div>
...
//       // Load employees progressively with photos
//       await this.graphService.getAllEmployees(
//         this.properties.selectedGroupId,
//         (employees) => {
//           // Update employees list as they come in
